...
nass <- occupants %>% mutate(casewgt = ratwgt)
...
nass <- nass %>% filter(year>=2000)
nass %>% count()

# filter cases with crash year >10 from model year after 2009
nass %>% filter(year>=2009 & ((nass$year - nass$modelyr) > 10)) %>% count()
nass <- nass %>% filter(year<2009 | ((nass$year - nass$modelyr) <= 10))
nass %>% count()

# filter for cases missing ISS
nass[is.na(nass$iss),] %>% count()
nass <- nass[!is.na(nass$iss),] 
nass %>% count()

# look at data
table(nass$dvtotal, nass$dvest, useNA = "ifany")
...
nass$rolled <- ifelse(nass$rollover > 0, 1, 0)
nass$roll_turns <- ifelse(nass$rollover > 0, nass$rollover, 0)

# set end-over-end code (98 to 17)
nass$roll_turns <- ifelse(nass$roll_turns==98, 17, nass$roll_turns)
...
nass$multicoll <- ifelse(is.na(nass$objcont2), 0, 1)

# flag entrapment
nass$entrapment <- ifelse(nass$entrap > 0, 1, 0)
...
nass$pdof_front <- ifelse((nass$pdof1>315 & nass$pdof1<=360) |
                            (nass$pdof1< 45), 1, 0)
nass$pdof_right <- ifelse(nass$pdof1>45 & nass$pdof1<135, 1, 0)
nass$pdof_rear <- ifelse(nass$pdof1>135 & nass$pdof1<225, 1, 0)
nass$pdof_left <- ifelse(nass$pdof1>225 & nass$pdof1<315, 1, 0)
...
nass$seat_side <- nass$seatpos %% 10
nass$seat_row <- floor(nass$seatpos / 10)

nass$pdof_nearside <- ifelse((nass$pdof_left == 1 & nass$seat_side == 1) 
                             |(nass$pdof_right == 1 & nass$seat_side == 3), 1, 0)
nass$pdof_farside <- ifelse((nass$pdof_left == 1 | nass$pdof_right == 1) & nass$pdof_nearside == 0, 1, 0)


# flag seat positions
nass$seat_dr <- ifelse(nass$seat_row == 1 & nass$seat_side == 1, 1, 0)
nass$seat_fmp <- ifelse(nass$seat_row == 1 & nass$seat_side == 2, 1, 0)
nass$seat_frp <- ifelse(nass$seat_row == 1 & nass$seat_side == 3, 1, 0)
nass$seat_rlp <- ifelse(nass$seat_row == 2 & nass$seat_side == 1, 1, 0)
nass$seat_rmp <- ifelse(nass$seat_row == 2 & nass$seat_side == 2, 1, 0)
nass$seat_rrp <- ifelse(nass$seat_row == 2 & nass$seat_side == 3, 1, 0)
...
#  then set death = 1
nass <- nass %>% mutate(death = ifelse(is.na(death) & injsev==4, 1, death))
occupants <- occupants %>% mutate(death = ifelse(is.na(death) & injsev==4, 1, death))

# make died flag
nass <- nass %>% mutate(died = ifelse((death>0 & death<96) | injsev==4, 1, 0))
occupants <- occupants %>% mutate(died = ifelse((death>0 & death<96) | injsev==4, 1, 0))

# if died is na, set to 0
nass <- nass %>% mutate(died = ifelse(is.na(died), 0, died))

# if occupant died and iss<24, set to 24
nass <- nass %>% mutate(iss = ifelse(died==1 & iss<24, 24, iss))
...
nass <- nass %>% left_join(vdied, by='veh_key')
...
xtabs(~nass$dvtotal + is.na(nass$abdeply), addNA = TRUE)

xtabs(~nass$dvtotal + is.na(nass$bagdeply), addNA = TRUE)

xtabs(~is.na(nass$abdeply) + is.na(nass$bagdeply), addNA = TRUE)

nass %>% group_by(abdeply) %>% summarise(n())
...
##############################################################
#  Identify other crash characteristics ------------------
#
...
nass <- nass %>% 
  select(key, year, age, sex, prop_restraint, any_restraint, abdeply, seat_row, entrapment,
         ...
         died, other_death, iss, iss16, mais, mais_head, mais_thorax, mais_abd, casewgt)

# write to file
nass %>% write.csv("./Data/NASS/NASS-2000_2015-unfiltered.csv", row.names = FALSE)
